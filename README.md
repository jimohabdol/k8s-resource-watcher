...
- Support for multiple resource types (ConfigMaps, Secrets, Deployments, Ingresses)
- Kubernetes-native deployment with RBAC support
- **Smart Field Watching**: Only alerts on important changes (no pod restart spam!)
- **Adaptive Cooldowns**: Dynamic notification frequency based on event patterns
- **Event Correlation**: Groups related events to reduce notification spam
- **Priority Processing**: Critical events (deletions) processed immediately
...
The watcher now intelligently filters changes based on resource type to eliminate notification spam:

#### **Deployments** - Only alerts on important changes:
- ✅ **Image changes** - Container image updates
- ✅ **Environment variables** - Env var modifications  
- ✅ **Volume mounts** - Storage configuration changes
- ✅ **Resource limits** - CPU/memory limit changes
- ✅ **Command/args** - Container startup command changes
- ✅ **Security context** - Security policy changes
- ✅ **Service account** - Identity changes
- ✅ **Node selector/affinity** - Scheduling changes
- ❌ **Pod restarts** - Status changes (filtered out)
- ❌ **Replica scaling** - Status changes (filtered out)

#### **ConfigMaps & Secrets** - Alerts on all data changes:
- ✅ **Data modifications** - Configuration updates
- ✅ **Data additions** - New configuration keys
- ✅ **Data deletions** - Removed configuration keys

#### **Ingresses** - Alerts on all spec changes:
- ✅ **Routing rules** - Host/path changes
- ✅ **TLS configuration** - Certificate changes
